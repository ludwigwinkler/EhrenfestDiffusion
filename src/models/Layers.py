...
def ncsn_conv3x3(in_planes, out_planes, stride=1, bias=True, dilation=1, init_scale=1.0, padding=1):
    """3x3 convolution with PyTorch initialization.

    Same as NCSNv1/NCSNv2.
    """
    init_scale = 1e-10 if init_scale == 0 else init_scale
...
class ConditionalResidualBlock(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, num_classes, resample=1, act=nn.ELU(), normalization=torch.nn.InstanceNorm2d, adjust_padding=False, dilation=None):
        super().__init__()
...
def get_timestep_embedding(timesteps, embedding_dim, max_positions=10000):
    assert len(timesteps.shape) == 1  # and timestep.dtype == tf.int32
    ...
    # emb = tf.cast(timestep, dtype=jnp.float32)[:, None] * emb[None, :]
