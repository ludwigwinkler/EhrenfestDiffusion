...
def get_inception_feature(
        ...
        verbose: bool = False,
        device: torch.device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('mps'),
    ...
    model = InceptionV3(block_idxs).to(device).float()
    # print(f"{list(model.parameters())[0].dtype=} {images.dtype=}")
    model.eval()
        ...
        with torch.no_grad():
            ...
            for feature, output, dim in zip(features, outputs, dims):
                if use_torch:
                    ...
                    feature[start: end] = output.view(-1, dim).cpu().float().numpy()
        start = end
...
def sqrt_newton_schulz(A, numIters, dtype=None):  # noqa
    with torch.no_grad():
        ...
        K = K.type(dtype).to(A.device)
        Z = Z.type(dtype).to(A.device)
...
def calculate_frechet_distance(
    ...
    if use_torch:
        ...
        # print(f"{mu1.device=} {mu2.device=}")
        diff = mu1 - mu2
        # Run 50 itrs of newton-schulz to get the matrix sqrt of
        # sigma1 dot sigma2
        covmean = sqrt_newton_schulz(sigma1.mm(sigma2).unsqueeze(0), 100)
        ...
        matrix_root = {'sqrtm': lambda x: linalg.sqrtm(x, disp=False)[0],
                       'fractional': lambda x: linalg.fractional_matrix_power(x, 0.5)}
        # covmean, _ = linalg.sqrtm(sigma1.dot(sigma2), disp=False)
        # covmean = linalg.fractional_matrix_power(sigma1.dot(sigma2), 0.5)
        covmean = matrix_root['fractional'](sigma1.dot(sigma2))
        if not np.isfinite(covmean).all():
            msg = ('fid calculation produces singular product; '
                   'adding %s to diagonal of cov estimates') % eps
            print(msg)
            offset = np.eye(sigma1.shape[0]) * eps
            covmean = matrix_root['fractional']((sigma1 + offset).dot(sigma2 + offset))
